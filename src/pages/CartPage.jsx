  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(t('cart.coupon_success'));
        setCouponCode("");
      } else {
        toast.error(data.error || t('cart.coupon_invalid'));
        setAppliedCoupon(null);
      }
    } catch (error) {
      toast.error(t('cart.connection_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4 bg-white">
        ...
        <h2 className="text-2xl font-black uppercase tracking-tight mb-2">{t('cart.empty_title')}</h2>
        <p className="text-gray-500 mb-8 font-light">{t('cart.empty_desc')}</p>
        <Link
          to="/"
                ...
                <div className="flex-1 flex flex-col justify-between py-1">
                  <div>
                    ...
                    <div className="text-xs text-gray-500 font-medium mb-4 space-y-1">
                      <p>{t('cart.size')} <span className="text-black">{item.selectedVariant?.size || t('cart.standard')}</span></p>
                      {item.selectedVariant?.color && (
                        <p>{t('cart.color')} <span className="text-black">{item.selectedVariant.color}</span></p>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-gray-50 p-6 md:p-8 sticky top-24 rounded-xl border border-gray-100">
              ...
              <div className="space-y-4 text-sm mb-6">
                ...
                <div className="flex justify-between text-gray-600 items-center">
                  ...
                  {SHIPPING_COST === 0 ? (
                    <span className="text-green-600 font-bold text-[10px] uppercase">{t('cart.free')}</span>
                  ) : (
                ...
                {!appliedCoupon ? (
                  ...
                  <div className="bg-white border border-green-200 p-3 rounded flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-2 text-green-700">
                      <Ticket size={16} />
                      <span className="text-xs font-bold">{appliedCoupon.code} {t('cart.applied')}</span>
                    </div>
